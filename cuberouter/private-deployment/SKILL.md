...
description: "Deploy cuberouter AI gateway from an offline tarball."
---

# CubeRouter Private Deployment (Offline)

Validated end-to-end 2026-08-31 on 4 KubeVirt VMs (10.66.2.207-210, Ubuntu 22.04.5). Full ops doc for that live deployment: /home/ubuntu/cbr-deploy/DEPLOYMENT.md.

## When to use
- Deploying cuberouter (AI API gateway, Go/Gin + React, new-api-based) from an offline tarball (e.g. /home/ubuntu/cuberouter-v1.0.0.tar.gz) to Ubuntu VMs without registry access.
- Upgrading (new tarball), scale-out (adding an app node), or troubleshooting an existing private deployment.

## Package anatomy (v1.0.0)
tarball → `docker-compose.yml` (app+redis+postgres single-node), `docker-compose.docs.yml` (docs-user :4080 + docs-admin :4081, same image twice), `scripts/gen-tls-cert.sh` (private CA + server cert; SANs mandatory; mode B = generate new root CA; mode A = sign with provided CA cert+key), `images.tar` (~840MB, 5 prebuilt OCI images from internal harbor — offline `docker load`).
Images: suanova/cuberouter:v1.0.0, library/postgres:15, library/redis:latest, suanova/cuberouter-docs-{user,admin}:v1.0.0.
Template passwords are all `123456` — always replace.

## Reference topology (4 VMs)
- VM1 data: postgres:15 + redis (requirepass + AOF); map 5432/6379; ufw-restrict later to app nodes.
- VM2 app node-1: NODE_NAME=cuberouter-node-1.
- VM3 app node-2: NODE_NAME=cuberouter-node-2, identical SESSION_SECRET, same DB/Redis.
- VM4 edge: nginx 443 TLS termination (SAN=edge IP) → upstream .2:3000,.3:3000 round-robin; 80→301; docs compose :4080/:4081.
- Do NOT co-locate LB and an app node (SPOF + tier anti-pattern).
All compose/nginx templates: `references/templates.md`.

## Procedure
1. Extract tarball locally; read compose + gen-tls-cert.sh before deploying.
2. Docker on each VM: `sudo apt-get install -y docker.io docker-compose-v2` (Ubuntu repo; 29.x on 22.04.5, current enough). download.docker.com is usually blocked on CN private nets (SSL conn reset); the aliyun docker-ce mirror lacks Release metadata — unusable by apt.
...
3. TLS on operator host: `./scripts/gen-tls-cert.sh --ips <edge-ip> --out certs` (mode B). Keep ca.crt — every client must trust it (system store or `curl --cacert`). Renew with mode A (`--ca-cert/--ca-key`) to keep the same CA.
4. Secrets (operator host, chmod 600): PG/Redis pw = `openssl rand -hex 24` (hex = DSN-URL safe); SESSION_SECRET=*** rand -hex 32`, byte-identical on ALL app nodes. One .env per node.
5. Write per-role compose (templates.md):
   - data: postgres user `cuberouter` (not root) + redis; healthchecks.
   - app: SQL_DSN/REDIS_CONN_STRING → data-node IP; TRUSTED_PROXIES=<subnet>; NODE_NAME per node; plain HTTP :3000 (TLS belongs at edge).
   - edge: docs compose as-is + nginx conf (SSE: proxy_buffering off, 600s timeouts, HTTP/1.1 + Upgrade map).
6. Distribute: `mkdir -p /opt/cuberouter && chown ubuntu` on each VM; scp bundle; `sudo docker load -i images.tar` per VM (L2-fast, ~2min/VM).
...
8. Bootstrap ROOT admin — there are NO default credentials:
   ...
   - Login: POST /api/user/login → token at `data.access_token` (NOT data.token). Auth header: `Authorization: Bearer ***
9. Verify:
   - /api/status 200 on each node direct + `curl --cacert ca.crt https://edge/api/status` via LB.
...
10. Harden (optional): ufw (templates.md appendix), change root password in console, SESSION_COOKIE_SECURE=true + SESSION_COOKIE_TRUSTED_URL=https://edge if Secure cookies are required (breaks direct-HTTP node sessions).

## API / product behavior notes (v1.0.0, verified)
...
## Upgrade (new tarball)
Extract → `docker load images.tar` (new tags) → update image refs in deploy-app.yml → on each app node `sudo docker compose --env-file <env> -f deploy-app.yml up -d` (volumes/env preserved; migrations run on start) → verify /api/status + logs. Data node untouched unless postgres/redis tags change.

## Scale-out (add app node)
Docker + image load on the new VM → deploy-app.yml + .env (copy SESSION_SECRET verbatim, new NODE_NAME) → up -d → add `server <ip>:3000` to the nginx upstream on edge → `nginx -t && systemctl reload nginx` → run the cross-node proof involving the new node.

## Reference
- `references/templates.md` — full compose files, nginx conf, .env layout, e2e verification pattern.
- Live deployment: /home/ubuntu/cbr-deploy/ (operator host) + /opt/cuberouter on each VM; ops doc /home/ubuntu/cbr-deploy/DEPLOYMENT.md.
